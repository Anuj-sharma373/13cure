<?php
/**
 * brtzhtheme functions and definitions
 *
 * Sets up the theme and provides some helper functions. Some helper functions
 * are used in the theme as custom template tags. Others are attached to action and
 * filter hooks in WordPress to change core functionality.
 *
 * The first function, brtzhtheme_setup(), sets up the theme by registering support
 ...
 *     remove_filter( 'excerpt_length', 'brtzhtheme_excerpt_length' );
 *     ...
 * }
 * </code>
 *
...
/* Tell WordPress to run brtzhtheme_setup() when the 'after_setup_theme' hook is run. */
add_action('after_setup_theme', 'brtzhtheme_setup');

if (!function_exists('brtzhtheme_setup')):
	/**
	 ...
	 *
	 * To override brtzhtheme_setup() in a child theme, add your own brtzhtheme_setup to your child theme's
	 * functions.php file.
	 *
	...
	function brtzhtheme_setup()
	{
		...
		add_theme_support(
			...
			array(
				array(
					'name' => __('Blue', 'brtzhtheme'),
					'slug' => 'blue',
					'color' => '#0066cc',
				),
				array(
					'name' => __('Black', 'brtzhtheme'),
					'slug' => 'black',
					'color' => '#000',
				),
				array(
					'name' => __('Medium Gray', 'brtzhtheme'),
					'slug' => 'medium-gray',
					'color' => '#666',
				),
				array(
					'name' => __('Light Gray', 'brtzhtheme'),
					'slug' => 'light-gray',
					'color' => '#f1f1f1',
				),
				array(
					'name' => __('White', 'brtzhtheme'),
					'slug' => 'white',
					'color' => '#fff',
				),
			)
		...
		if (version_compare($GLOBALS['wp_version'], '4.6', '<')) {
			load_theme_textdomain('brtzhtheme', get_template_directory() . '/languages');
		}

		// This theme uses wp_nav_menu() in one location.
		register_nav_menus(
			array(
				'primary' => __('Primary Navigation', 'brtzhtheme'),
			)
		);
		...
		$custom_header_support = array(
			...
			/**
			 * Filters the BRT Zh Theme default header image width.
			 *
			 * @since BRT Zh Theme 1.0
			 *
			 * @param int The default header image width in pixels. Default 940.
			 */
			'width' => apply_filters('brtzhtheme_header_image_width', 940),
			/**
			 * Filters the BRT Zh Theme default header image height.
			 *
			 * @since BRT Zh Theme 1.0
			 *
			 * @param int The default header image height in pixels. Default 198.
			 */
			'height' => apply_filters('brtzhtheme_header_image_height', 198),
			// Support flexible heights.
			'flex-height' => true,
			// Don't support text inside the header image.
			'header-text' => false,
			// Callback for styling the header preview in the admin.
			'admin-head-callback' => 'brtzhtheme_admin_header_style',
		);
		...
		register_default_headers(
			array(
				'berries' => array(
					...
					'description' => __('Berries', 'brtzhtheme'),
				),
				'cherryblossom' => array(
					'url' => '%s/images/headers/cherryblossoms.jpg',
					'thumbnail_url' => '%s/images/headers/cherryblossoms-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Cherry Blossoms', 'brtzhtheme'),
				),
				'concave' => array(
					'url' => '%s/images/headers/concave.jpg',
					'thumbnail_url' => '%s/images/headers/concave-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Concave', 'brtzhtheme'),
				),
				'fern' => array(
					'url' => '%s/images/headers/fern.jpg',
					'thumbnail_url' => '%s/images/headers/fern-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Fern', 'brtzhtheme'),
				),
				'forestfloor' => array(
					'url' => '%s/images/headers/forestfloor.jpg',
					'thumbnail_url' => '%s/images/headers/forestfloor-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Forest Floor', 'brtzhtheme'),
				),
				'inkwell' => array(
					'url' => '%s/images/headers/inkwell.jpg',
					'thumbnail_url' => '%s/images/headers/inkwell-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Inkwell', 'brtzhtheme'),
				),
				'path' => array(
					'url' => '%s/images/headers/path.jpg',
					'thumbnail_url' => '%s/images/headers/path-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Path', 'brtzhtheme'),
				),
				'sunset' => array(
					'url' => '%s/images/headers/sunset.jpg',
					'thumbnail_url' => '%s/images/headers/sunset-thumbnail.jpg',
					/* translators: Header image description. */
					'description' => __('Sunset', 'brtzhtheme'),
				),
			)
		);
	}
...
if (!function_exists('brtzhtheme_admin_header_style')):
	/**
	 * Style the header image displayed on the Appearance > Header admin panel.
	 *
	 * Referenced via add_custom_image_header() in brtzhtheme_setup().
	 *
	 * @since BRT Zh Theme 1.0
	 */
	function brtzhtheme_admin_header_style()
	{
		?>
		<style type="text/css" id="brtzhtheme-admin-header-css">
...
if (!function_exists('brtzhtheme_header_image')):
	/**
	 * Custom header image markup displayed.
	 *
	 * @since BRT Zh Theme 4.0
	 */
	function brtzhtheme_header_image()
	{
		$attrs = array(
 ...
 */
function brtzhtheme_page_menu_args($args)
{
...
add_filter('wp_page_menu_args', 'brtzhtheme_page_menu_args');
 ...
 */
function brtzhtheme_excerpt_length($length)
{
	return 40;
}
add_filter('excerpt_length', 'brtzhtheme_excerpt_length');

if (!function_exists('brtzhtheme_continue_reading_link')):
	/**
	 ...
	 */
	function brtzhtheme_continue_reading_link()
	{
		return ' <a href="' . esc_url(get_permalink()) . '">' . __('Continue reading <span class="meta-nav">&rarr;</span>', 'brtzhtheme') . '</a>';
	}
endif;

/**
 * Replace "[...]" with an ellipsis and brtzhtheme_continue_reading_link().
 *
 ...
 */
function brtzhtheme_auto_excerpt_more($more)
{
	if (!is_admin()) {
		return ' &hellip;' . brtzhtheme_continue_reading_link();
	}
	return $more;
}
add_filter('excerpt_more', 'brtzhtheme_auto_excerpt_more');
 ...
 */
function brtzhtheme_custom_excerpt_more($output)
{
	if (has_excerpt() && !is_attachment() && !is_admin()) {
		$output .= brtzhtheme_continue_reading_link();
	}
	return $output;
}
add_filter('get_the_excerpt', 'brtzhtheme_custom_excerpt_more');

/**
 * Remove inline styles printed when the gallery shortcode is used.
 *
 * Galleries are styled by the theme in BRT Zh Theme's style.css. This is just
 ...
 *
 * @since BRT Zh Theme 1.0
 * @deprecated Deprecated in BRT Zh Theme 1.2 for WordPress 3.1
 *
 * @return string The gallery style filter, with the styles themselves removed.
 */
function brtzhtheme_remove_gallery_css($css)
{
	return preg_replace("#<style type='text/css'>(.*?)</style>#s", '', $css);
}
// Backward compatibility with WordPress 3.0.
if (version_compare($GLOBALS['wp_version'], '3.1', '<')) {
	add_filter('gallery_style', 'brtzhtheme_remove_gallery_css');
}

if (!function_exists('brtzhtheme_comment')):
	/**
	 * Template for comments and pingbacks.
	 *
	 * To override this walker in a child theme without modifying the comments template
	 * simply create your own brtzhtheme_comment(), and that function will be used instead.
	 *
	 ...
	 */
	function brtzhtheme_comment($comment, $args, $depth)
	{
		...
		switch ($comment->comment_type):
				...
				<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
					<div id="comment-<?php comment_ID(); ?>">
						<div class="comment-author vcard">
							...
							printf(__('%s <span class="says">says:</span>', 'brtzhtheme'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
							?>
						...
						if ($commenter['comment_author_email']) {
							$moderation_note = __('Your comment is awaiting moderation.', 'brtzhtheme');
						} else {
							$moderation_note = __('Your comment is awaiting moderation. This is a preview; your comment will be visible after it has been approved.', 'brtzhtheme');
						}
						?>
						...
						<div class="comment-meta commentmetadata"><a
								...
								printf(__('%1$s at %2$s', 'brtzhtheme'), get_comment_date(), get_comment_time());
								?>
							</a>
							<?php
							edit_comment_link(__('(Edit)', 'brtzhtheme'), ' ');
							?>
			...
			case 'trackback':
				...
				<li class="post pingback">
					<p><?php _e('Pingback:', 'brtzhtheme'); ?>
						<?php comment_author_link(); ?> 				<?php edit_comment_link(__('(Edit)', 'brtzhtheme'), ' '); ?>
					</p>
					<?php
					break;
		endswitch;
 ...
 *
 * To override brtzhtheme_widgets_init() in a child theme, remove the action hook and add your own
 ...
 */
function brtzhtheme_widgets_init()
{
	// Area 1, located at the top of the sidebar.
	register_sidebar(
		array(
			'name' => __('Primary Widget Area', 'brtzhtheme'),
			'id' => 'primary-widget-area',
			'description' => __('Add widgets here to appear in your sidebar.', 'brtzhtheme'),
	...
	register_sidebar(
		array(
			'name' => __('Secondary Widget Area', 'brtzhtheme'),
			'id' => 'secondary-widget-area',
			'description' => __('An optional secondary widget area, displays below the primary widget area in your sidebar.', 'brtzhtheme'),
			'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
			'after_widget' => '</li>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
	...
	register_sidebar(
		array(
			'name' => __('First Footer Widget Area', 'brtzhtheme'),
			'id' => 'first-footer-widget-area',
			'description' => __('An optional widget area for your site footer.', 'brtzhtheme'),
			'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
			'after_widget' => '</li>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
	...
	register_sidebar(
		array(
			'name' => __('Second Footer Widget Area', 'brtzhtheme'),
	...
	register_sidebar(
		array(
			'name' => __('Third Footer Widget Area', 'brtzhtheme'),
	...
	register_sidebar(
		array(
			'name' => __('Fourth Footer Widget Area', 'brtzhtheme'),
...
/** Register sidebars by running brtzhtheme_widgets_init() on the widgets_init hook. */
add_action('widgets_init', 'brtzhtheme_widgets_init');
 ...
 * to remove the default style. Using BRT Zh Theme 1.2 in WordPress 3.0 will show the styles,
 * but they won't have any effect on the widget in default BRT Zh Theme styling.
 *
 * @since BRT Zh Theme 1.0
 */
function brtzhtheme_remove_recent_comments_style()
{
	add_filter('show_recent_comments_widget_style', '__return_false');
}
add_action('widgets_init', 'brtzhtheme_remove_recent_comments_style');

if (!function_exists('brtzhtheme_posted_on')):
	/**
	 * Print HTML with meta information for the current post-date/time and author.
	 *
	 * @since BRT Zh Theme 1.0
	 */
	function brtzhtheme_posted_on()
	{
		printf(
			/* translators: 1: CSS classes, 2: Date, 3: Author display name. */
			__('<span class="%1$s">Posted on</span> %2$s <span class="meta-sep">by</span> %3$s', 'brtzhtheme'),
			...
			sprintf(
				...
				/* translators: %s: Author display name. */
				esc_attr(sprintf(__('View all posts by %s', 'brtzhtheme'), get_the_author())),
				get_the_author()
			)
		);
	}
...
if (!function_exists('brtzhtheme_posted_in')):
	...
	function brtzhtheme_posted_in()
	{
		...
		if ($tags_list && !is_wp_error($tags_list)) {
			...
			$posted_in = __('This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'brtzhtheme');
		} elseif (is_object_in_taxonomy(get_post_type(), 'category')) {
			/* translators: 1: Category name, 3: Post permalink, 4: Post title. */
			$posted_in = __('This entry was posted in %1$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'brtzhtheme');
		} else {
			/* translators: 3: Post permalink, 4: Post title. */
			$posted_in = __('Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'brtzhtheme');
		}
 ...
 *
 * @since BRT Zh Theme 1.6.
 *
 * @return array List of image IDs from the post gallery.
 */
function brtzhtheme_get_gallery_images()
{
	$images = array();
 ...
 *
 * @since BRT Zh Theme 2.4
 *
 * @param array $args Arguments for tag cloud widget.
 * @return array The filtered arguments for tag cloud widget.
 */
function brtzhtheme_widget_tag_cloud_args($args)
{
	$args['largest'] = 22;
	$args['smallest'] = 8;
	$args['unit'] = 'pt';
...
add_filter('widget_tag_cloud_args', 'brtzhtheme_widget_tag_cloud_args');

/**
 * Enqueue scripts and styles for front end.
 *
 * @since BRT Zh Theme 2.6
 */
function brtzhtheme_scripts_styles()
{
	// Theme block stylesheet.
	wp_enqueue_style('brtzhtheme-block-style', get_template_directory_uri() . '/blocks.css', array(), '20230627');
}
add_action('wp_enqueue_scripts', 'brtzhtheme_scripts_styles');
 ...
 * @since BRT Zh Theme 2.6
 */
function brtzhtheme_block_editor_styles()
{
	// Block styles.
	wp_enqueue_style('brtzhtheme-block-editor-style', get_template_directory_uri() . '/editor-blocks.css', array(), '20230627');
}
add_action('enqueue_block_editor_assets', 'brtzhtheme_block_editor_styles');
...
//   }
	...
	function custom_post_type_services()
	{
		...
		$labels = array(
			'name' => _x('Our Services', 'Post Type General Name', 'BRT_Zh_Theme'),
			'singular_name' => _x('Service', 'Post Type Singular Name', 'BRT_Zh_Theme'),
			'menu_name' => __('Services', 'BRT_Zh_Theme'),
			'parent_item_colon' => __('Parent Service', 'BRT_Zh_Theme'),
			'all_items' => __('All Services', 'BRT_Zh_Theme'),
			'view_item' => __('View Service', 'BRT_Zh_Theme'),
			'add_new_item' => __('Add New Service', 'BRT_Zh_Theme'),
			'add_new' => __('Add New', 'BRT_Zh_Theme'),
			'edit_item' => __('Edit Service', 'BRT_Zh_Theme'),
			'update_item' => __('Update Service', 'BRT_Zh_Theme'),
			'search_items' => __('Search Services', 'BRT_Zh_Theme'),
			'not_found' => __('Not Found', 'BRT_Zh_Theme'),
			'not_found_in_trash' => __('Not found in Trash', 'BRT_Zh_Theme'),
		);

		$args = array(
			'label' => __('services', 'BRT_Zh_Theme'),
			'description' => __('Services', 'BRT_Zh_Theme'),
			'labels' => $labels,
			'supports' => array('title', 'editor', 'excerpt', 'author', 'thumbnail', 'comments', 'revisions', 'custom-fields'),
			'taxonomies' => array('category'),
			'hierarchical' => false,
	...
	function custom_post_type_therapist()
	{
		...
		$labels = array(
			'name' => _x('Our Therapists', 'Post Type General Name', 'BRT_Zh_Theme'),
			'singular_name' => _x('Therapist', 'Post Type Singular Name', 'BRT_Zh_Theme'),
			'menu_name' => __('Therapists', 'BRT_Zh_Theme'),
			'parent_item_colon' => __('Parent Therapist', 'BRT_Zh_Theme'),
			'all_items' => __('All Therapists', 'BRT_Zh_Theme'),
			'view_item' => __('View Therapist', 'BRT_Zh_Theme'),
			'add_new_item' => __('Add New Therapist', 'BRT_Zh_Theme'),
			'add_new' => __('Add New', 'BRT_Zh_Theme'),
			'edit_item' => __('Edit Therapist', 'BRT_Zh_Theme'),
			'update_item' => __('Update Therapist', 'BRT_Zh_Theme'),
			'search_items' => __('Search Therapists', 'BRT_Zh_Theme'),
			'not_found' => __('Not Found', 'BRT_Zh_Theme'),
			'not_found_in_trash' => __('Not found in Trash', 'BRT_Zh_Theme'),
		);

		$args = array(
			'label' => __('therapist', 'BRT_Zh_Theme'),
			'description' => __('therapist', 'BRT_Zh_Theme'),
			'labels' => $labels,
			'supports' => array('title', 'editor', 'excerpt', 'author', 'thumbnail', 'comments', 'revisions', 'custom-fields'),
			'taxonomies' => array('category'),
			'hierarchical' => false,
