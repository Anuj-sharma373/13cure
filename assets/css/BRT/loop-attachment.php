	...
	while ( have_posts() ) :
		...
		?>
				...
				if ( ! empty( $post->post_parent ) ) :
					...
					$post_title = sprintf( __( 'Go to %s', 'brttheme' ), strip_tags( get_the_title( $post->post_parent ) ) );
					?>
					<p class="page-title"><a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php echo esc_attr( $post_title ); ?>" rel="gallery">
						...
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'brttheme' ), get_the_title( $post->post_parent ) );
						?>
					</a></p>
				<?php endif; ?>
					...
					<div class="entry-meta">
						<?php
							printf(
								...
								__( '<span class="%1$s">By</span> %2$s', 'brttheme' ),
						...
						?>
							...
							printf(
								...
								__( '<span class="%1$s">Published</span> %2$s', 'brttheme' ),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf(
									...
									__( 'Full size is %s pixels', 'brttheme' ),
									sprintf(
										'<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										esc_url( wp_get_attachment_url() ),
										esc_attr( __( 'Link to full-size image', 'brttheme' ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
		...
		if ( wp_attachment_is_image() ) :
			...
			?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment">
							<?php
							/**
							 * Filters the BRT Theme default attachment width.
							 *
							 * @since BRT Theme 1.0
							 *
							 * @param int The default attachment width in pixels. Default 900.
							 */
							$attachment_width = apply_filters( 'brttheme_attachment_size', 900 );
							/**
							 * Filters the BRT Theme default attachment height.
							 *
							 * @since BRT Theme 1.0
							 *
							 * @param int The default attachment height in pixels. Default 900.
							 */
							$attachment_height = apply_filters( 'brttheme_attachment_height', 900 );
	...
	<?php endif; ?>
		...
		?>
					...
					<div class="entry-utility">
						<?php brttheme_posted_in(); ?>
						<?php edit_post_link( __( 'Edit', 'brttheme' ), ' <span class="edit-link">', '</span>' ); ?>
					</div><!-- .entry-utility -->
				</div><!-- #post-<?php the_ID(); ?> -->
